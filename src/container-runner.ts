...
function buildVolumeMounts(
  ...
  // Read existing settings or start with defaults
  let settings: { env?: Record<string, string> } = {};
  if (fs.existsSync(settingsFile)) {
    try {
      settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    } catch {
      // Ignore parse errors — overwrite with defaults below
    }
  }
  if (!settings.env) {
    settings.env = {
  ...
  // Always refresh the proxy URL — it depends on the bridge IP which can change
